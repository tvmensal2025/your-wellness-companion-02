...
const HeroCaloriesCard: React.FC<HeroCaloriesProps> = ({ 
  ...
  const getObjectiveInfo = () => {
    switch (objective) {
      case NutritionObjective.LOSE: return { label: 'Emagrecendo', color: 'text-blue-400', icon: TrendingDown };
      case NutritionObjective.GAIN: return { label: 'Ganhando', color: 'text-green-400', icon: TrendingUp };
      case NutritionObjective.LEAN_MASS: return { label: 'Definindo', color: 'text-purple-400', icon: Zap };
      default: return { label: 'Mantendo', color: 'text-emerald-400', icon: Target };
    }
  };
        ...
        <div className="relative">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <motion.span 
              className={cn(
                "text-4xl font-bold",
                isOverTarget ? "text-red-400" : "text-white"
              )}
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
            ...
            <span className="text-slate-400 text-sm">de {target} kcal</span>
            <div className={cn(
...
const MacroPills: React.FC<MacroPillsProps> = ({ protein, carbs, fat }) => {
  const macros = [
    { label: 'Proteína', current: protein.current, target: protein.target, color: 'from-orange-400 to-red-500', bg: 'bg-orange-500/10' },
    { label: 'Carbos', current: carbs.current, target: carbs.target, color: 'from-blue-400 to-cyan-500', bg: 'bg-blue-500/10' },
    { label: 'Gorduras', current: fat.current, target: fat.target, color: 'from-yellow-400 to-amber-500', bg: 'bg-yellow-500/10' },
  ];
        ...
        return (
          ...
          >
            ...
            <div className="h-1.5 bg-muted/30 rounded-full overflow-hidden">
              <motion.div
                className={cn("h-full rounded-full bg-gradient-to-r", macro.color)}
                initial={{ width: 0 }}
                animate={{ width: `${pct}%` }}
      ...
      {MEAL_CONFIG.map((config, idx) => {
        ...
        return (
          <motion.div
            ...
            className={cn(
              "flex-shrink-0 w-20 cursor-pointer transition-transform hover:scale-105",
              !isLogged && "opacity-60"
            )}
          >
            {/* Circle com gradiente ou vazio */}
            <div className={cn(
              "relative w-20 h-20 rounded-full p-1 mx-auto",
              isLogged 
                ? `bg-gradient-to-br ${config.gradient} shadow-lg ${config.bgGlow}` 
                : "bg-gradient-to-br from-slate-700 to-slate-800"
            )}>
              <div className={cn(
                "w-full h-full rounded-full flex items-center justify-center",
                isLogged ? "bg-slate-900" : "bg-slate-800/80"
              )}>
                {isLogged ? (
...
const MealDetailCard: React.FC<MealDetailCardProps> = ({ meal, config, onClose }) => {
...
const WeekHistory: React.FC<WeekHistoryProps> = ({ weekData }) => {
          ...
          {days.map((day, idx) => {
            ...
            return (
              <div key={idx} className="flex flex-col items-center gap-1.5">
                ...
                <div className="relative w-8 h-16 bg-muted/30 rounded-full overflow-hidden">
                  <motion.div
                    className={cn(
                      ...
                      pct >= 80 ? "bg-gradient-to-t from-emerald-500 to-emerald-400" :
                      pct >= 50 ? "bg-gradient-to-t from-amber-500 to-amber-400" :
                      pct > 0 ? "bg-gradient-to-t from-red-500 to-red-400" :
                      "bg-transparent"
                    )}
                    initial={{ height: 0 }}
                  ...
                  {/* Indicador de hoje */}
                  {isToday && (
                    <div className="absolute inset-0 ring-2 ring-primary ring-offset-1 ring-offset-background rounded-full" />
                  )}
                </div>
                ...
                <span className={cn(
                  "text-[10px] font-medium",
                  logged >= 3 ? "text-emerald-500" : "text-muted-foreground"
                )}>
                  {logged > 0 ? logged : '-'}
                </span>
              </div>
...
const SofiaTip: React.FC<{ mealsLogged: number }> = ({ mealsLogged }) => {
  const getMessage = () => {
    const hour = new Date().getHours();
    
    if (mealsLogged === 0) {
      if (hour < 10) return { text: "Bom dia! Me manda uma foto do café ☕", emoji: "👋" };
      if (hour < 14) return { text: "Hora do almoço! Registra pra mim? 🍽️", emoji: "😊" };
      return { text: "Como foi a alimentação hoje?", emoji: "🤔" };
    }
    
    if (mealsLogged === 4) {
      return { text: "Dia completo! Você arrasou! 🎉", emoji: "🏆" };
    }
    
    const pending = 4 - mealsLogged;
    if (pending === 1) return { text: "Falta só mais uma refeição!", emoji: "💪" };
    return { text: `Faltam ${pending} refeições hoje`, emoji: "📸" };
  };
...
export const SofiaNutricionalRedesigned: React.FC<SofiaNutricionalRedesignedProps> = ({ userId }) => {
  // Estados
    ...
    if (data) {
      const formattedMeals: MealData[] = data.map((item: any) => ({
        ...
        foods_detected: Array.isArray(item.foods_detected) 
          ? item.foods_detected.map((f: any) => typeof f === 'string' ? f : f.name || f.food || 'Alimento')
          : [],
  ...
  const loadWeekData = async () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const { data } = await supabase
        .from('sofia_food_analysis')
        .select('total_calories, calories')
        .eq('user_id', userId)
  ...
  return (
    <div className="space-y-4 pb-4">
      ...
      <HeroCaloriesCard
        ...
        target={userGoals?.target_calories || 2000}
        objective={userGoals?.objective || NutritionObjective.MAINTAIN}
        streak={streak}
      ...
      {/* Macro Pills */}
      <MacroPills
        protein={{ current: dailyStats.protein, target: userGoals?.target_protein || 150 }}
        carbs={{ current: dailyStats.carbs, target: userGoals?.target_carbs || 250 }}
        fat={{ current: dailyStats.fat, target: userGoals?.target_fat || 65 }}
      />
