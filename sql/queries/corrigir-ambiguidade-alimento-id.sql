-- ========================================
-- CORREÇÃO DEFINITIVA: AMBIGUIDADE ALIMENTO_ID
-- INSERIR DADOS SEM ERROS
-- ========================================
...
-- 2. SEGUNDO: INSERIR VALORES NUTRICIONAIS (SEM AMBIGUIDADE)
DO $$
DECLARE
    current_alimento_id INTEGER;
    valor_count INTEGER;
BEGIN
    -- Para cada alimento, verificar se já tem valores nutricionais
    FOR current_alimento_id IN SELECT id FROM alimentos_completos ORDER BY id LOOP
        -- CORREÇÃO: Usar alias para evitar ambiguidade
        SELECT COUNT(*) INTO valor_count FROM valores_nutricionais_completos vnc WHERE vnc.alimento_id = current_alimento_id;
        ...
        IF valor_count = 0 THEN
            ...
            WHERE ac.id = current_alimento_id;
            
            RAISE NOTICE '✅ Valores nutricionais inseridos para alimento ID %', current_alimento_id;
        ELSE
            RAISE NOTICE '⚠️ Valores nutricionais já existem para alimento ID %', current_alimento_id;
        END IF;
    END LOOP;
END $$;
...
-- 4. QUARTO: INSERIR PRINCÍPIOS ATIVOS
DO $$
DECLARE
...
SELECT '✅ AMBIGUIDADE CORRIGIDA E DADOS INSERIDOS!' as status;
SELECT '🍎 23 alimentos medicinais verificados' as alimentos;
