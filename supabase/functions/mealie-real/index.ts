  ...
  try {
    ...
    for(let i = 0; i < basicRecipes.length; i++){
      try {
        ...
        console.log(`⚠️ Erro na receita: ${error?.message || error}`);
      }
    }
...
function mapCategory(category: any): string {
  if (!category) return '';
  
  if (Array.isArray(category)) {
    return category.map((c: any)=>c.name || c.slug || c).join(', ');
  }
  
  return category.name || category.slug || category || '';
}

function mapTags(tags: any): string[] {
  if (!tags || !Array.isArray(tags)) return [];
  
  return tags.map((tag: any)=>tag.name || tag.slug || tag).filter(Boolean);
}

function mapIngredients(ingredients: any): any[] {
...
function mapNutrition(nutrition: any, description = ''): any {
...
function mapMealieRecipe(raw: any, tipoRefeicao = ''): any {
  // Preparar instruções completas
  const preparoCompleto = (raw.recipeInstructions || []).map((inst: any, index: number)=>{
    if (inst.title) {
      return `\n## ${inst.title}\n${index + 1}. ${inst.text || ''}`;
    }
  ...
  const infoExtras = {
    ...
    ingredientes_texto: (raw.recipeIngredient || []).map((ing: any) =>
...
function filterByRestrictions(recipes: any[], restrictions: any): any[] {
  ...
  const filteredRecipes = recipes.filter((recipe: any)=>{
    ...
    if (recipe.tags && Array.isArray(recipe.tags)) {
      tagsText = recipe.tags.map((t: any)=>t.name || t.slug || t).join(' ').toLowerCase();
    }
    
    let categoryText = '';
    if (recipe.recipeCategory && Array.isArray(recipe.recipeCategory)) {
      categoryText = recipe.recipeCategory.map((c: any)=>c.name || c.slug || c).join(' ').toLowerCase();
    }
    ...
    for (const restriction of normalizedRestrictions) {
      ...
      const keywords = (RESTRICTION_MAPPING as any)[restriction] || [restriction];
...
function prioritizeByPreferences(recipes: any[], preferences: any): any[] {
  ...
  if (normalizedPreferences.length === 0) {
    ...
    return recipes.map((recipe: any) => ({ ...recipe, score: 0 }));
  }
  ...
  const scoredRecipes = recipes.map((recipe: any)=>{
    ...
    for (const preference of normalizedPreferences) {
      ...
      const keywords = (PREFERENCE_MAPPING as any)[preference] || [preference];
  ...
  const recipesWithScore = scoredRecipes.filter((r: any) => r.score > 0);
...
function selectRecipeForMeal(recipes: any[], mealType: string, targetCalories = 0): any {
  ...
  const categories = (mealTypeCategories as any)[mealType] || { strongPositive: [], weakPositive: [], negative: [] };
  ...
  const sortedRecipes = scoredRecipes.sort((a: any, b: any) => (b.mealScore || 0) - (a.mealScore || 0));
  
  // Filtrar receitas adequadas (pontuação > 0)
  let suitableRecipes = sortedRecipes.filter((r: any) => (r.mealScore || 0) > 0);
...
serve(async (req)=>{
  ...
  try {
    ...
    const caloriasPorRefeicao: any = {};
    refeicoesSelecionadas.forEach((refeicao: string) => {
      const percentual = (distribuicaoCaloricas as any)[refeicao] || (100 / refeicoesSelecionadas.length);
    ...
    for(let day = 1; day <= dias; day++){
      ...
      let selectedMealTypes = refeicoesSelecionadas
        .map((meal: string)=>mealTypeMapping[meal] || meal)
        .filter(Boolean);
      ...
      for (const mealType of selectedMealTypes) {
        ...
        const availableRecipes = prioritizedRecipes.filter((r: any)=>!usedRecipes.has(r.id));
