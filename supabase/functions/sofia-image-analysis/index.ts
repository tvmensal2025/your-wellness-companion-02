...
async function tryYoloDetect(imageUrl: string): Promise<{ 
  ...
  console.log(`🦾 YOLO: Detecção rápida em ${yoloServiceUrl}...`);
  
  // Configurações otimizadas para VELOCIDADE
  const confidenceLevel = 0.35; // Balanceado
  
  try {
    // Timeout agressivo de 8s (removido health check para velocidade)
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 8000);
    ...
    const err = error as Error;
    console.log(`⚠️ YOLO: ${err.name === 'AbortError' ? 'Timeout' : err.message}`);
    return null;
  }
}
  ...
  try {
    ...
    if (ensembleResults.length > 0) {
      ...
      const finalObjects = Array.from(uniqueObjects.values())
        .filter((obj: any) => obj.count >= 1) // Pelo menos 1 modelo detectou
        .map((obj: any) => ({
          class_name: obj.class_name,
          score: (obj.scores as number[]).reduce((a: number, b: number) => a + b, 0) / obj.scores.length,
          name: YOLO_CLASS_MAP[obj.class_name] || ''
        }))
        .filter((obj: any) => !!obj.name && obj.score >= 0.6);
      
      console.log(`🔄 Ensemble: ${finalObjects.length} objetos únicos detectados`);
      
      return {
        objects: finalObjects,
        confidence: finalObjects.reduce((sum: number, obj: any) => sum + obj.score, 0) / finalObjects.length,
...
Ou você pode me contar o que está comendo! 😉✨`,
    ...
    if (comboDetected) {
      ...
      foodList = Array.isArray(detectedFoods) && detectedFoods.length > 0 && typeof detectedFoods[0] === 'object'
        ? (detectedFoods as Array<{nome: string; quantidade: number}>).map((food) => `• ${food.nome} – ${food.quantidade}g`).join('\n')
