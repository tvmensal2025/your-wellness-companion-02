-- ========================================
-- LIMPEZA COMPLETA DE DADOS SIMULADOS
-- Remove TODOS os dados fictícios do sistema
-- ========================================

-- 1. REMOVER CHALLENGES SIMULADOS (manter apenas dados reais do usuário)
...
DELETE FROM anamnesis WHERE user_id IN (
  SELECT user_id FROM profiles WHERE 
  full_name LIKE '%dsf%' OR
  full_name LIKE '%dsfqw%' OR
  full_name LIKE '%dsfdfg%' OR
...
-- 5. LIMPAR TABELAS DE DADOS SIMULADOS
DELETE FROM valores_nutricionais_completos;

-- 6. RESETAR SEQUÊNCIAS
