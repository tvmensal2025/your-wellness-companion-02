...
DROP POLICY IF EXISTS "Permitir leitura pública da tabela taco_foods" ON taco_foods;
CREATE POLICY "taco_foods_public_read" ON taco_foods FOR SELECT USING (true);

-- Política para permitir inserção apenas para administradores
DROP POLICY IF EXISTS "Permitir inserção apenas para admins" ON taco_foods;
CREATE POLICY "taco_foods_admin_insert" ON taco_foods FOR INSERT
WITH CHECK (
  EXISTS (
