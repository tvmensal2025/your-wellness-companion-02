...
create policy if not exists "Users select own premium reports"
  on public.premium_medical_reports for select
  using (auth.uid() = user_id OR public.is_admin_user(auth.uid()));

create policy if not exists "Users insert own premium reports"
  on public.premium_medical_reports for insert
  with check (auth.uid() = user_id);

create policy if not exists "Users update own premium reports"
  on public.premium_medical_reports for update
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);

create policy if not exists "Admins can manage premium reports"
  on public.premium_medical_reports for all
  using (public.is_admin_user(auth.uid()))
  with check (public.is_admin_user(auth.uid()));

-- Updated_at trigger
create trigger if not exists trg_premium_reports_updated_at
...
create policy if not exists "Users select own premium report events"
  on public.premium_report_events for select
  using (auth.uid() = user_id OR public.is_admin_user(auth.uid()));

create policy if not exists "Users insert own premium report events"
  on public.premium_report_events for insert
  with check (auth.uid() = user_id);

create policy if not exists "Admins manage premium report events"
...
create policy if not exists "Users can upload medical reports to their folder"
...
create policy if not exists "Users can read their medical reports"
...
create policy if not exists "Users can update their medical reports"
...
create policy if not exists "Users can delete their medical reports"
...
create policy if not exists "Admins can read all medical reports files"
  on storage.objects for select
