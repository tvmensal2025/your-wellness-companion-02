-- RECUPERAÇÃO COMPLETA DO SISTEMA DE METAS
...
-- Verificar tabela goal_categories
INSERT INTO goal_categories (name, icon, color, is_active)
VALUES 
    ('Peso Ideal', '⚖️', '#FF6B6B', true),
    ('Exercícios Diários', '💪', '#4ECDC4', true),
    ('Alimentação Saudável', '🥗', '#95E1D3', true),
    ('Saúde Mental', '🧠', '#96CEB4', true),
    ('Hidratação', '💧', '#06b6d4', true),
    ('Sono Reparador', '😴', '#8b5cf6', true),
    ('Resistência Física', '🏃', '#f59e0b', true),
    ('Flexibilidade', '🤸', '#ec4899', true)
...
INSERT INTO user_goals (
    user_id, title, description, category_id, target_value, unit, 
    difficulty, estimated_points, status, current_value
)
SELECT 
    (SELECT id FROM auth.users LIMIT 1),
    'Meta de Teste - Peso Ideal',
    'Alcançar o peso ideal através de alimentação balanceada e exercícios',
    (SELECT id FROM goal_categories WHERE name = 'Peso Ideal' LIMIT 1),
    70.0,
    'kg',
    'medio',
    50,
    ...
    0.0
WHERE NOT EXISTS (
    SELECT 1 FROM user_goals WHERE title = 'Meta de Teste - Peso Ideal'
);