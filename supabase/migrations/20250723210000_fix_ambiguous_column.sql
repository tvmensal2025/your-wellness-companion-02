-- Corrigir erro de coluna ambígua no trigger generate_weekly_analysis
...
BEGIN
  ...
  SELECT wm.peso_kg INTO v_peso_inicial
  FROM weight_measurements wm
  WHERE wm.user_id = NEW.user_id 
    AND wm.measurement_date >= v_semana_inicio 
    AND wm.measurement_date <= v_semana_fim
