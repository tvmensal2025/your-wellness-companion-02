// TESTE CORRIGIDO - Execute no console do navegador
console.log('🚀 TESTE CORRIGIDO DO GOOGLE FIT');
...
async function testeCorrigido() {
  ...
  try {
    ...
    // Tentar usar o Supabase global primeiro
    let supabaseClient = window.supabase;
    
    if (!supabaseClient) {
      console.log('❌ Supabase global não disponível');
      console.log('💡 Tentando importar...');
      
      // Se não estiver global, tentar importar
      try {
...
CREATE POLICY "Users can manage their own Google Fit tokens" ON google_fit_tokens
  FOR ALL USING (auth.uid() = user_id);
    ...
    const { data: fitData, error: fitError } = await supabaseClient
...
CREATE POLICY "Users can manage their own Google Fit data" ON google_fit_data
  FOR ALL USING (auth.uid() = user_id);
    ...
    const { data, error } = await supabaseClient.functions.invoke('google-fit-token', {
    ...
    console.error('💥 Erro no teste corrigido:', error);
  }
}
