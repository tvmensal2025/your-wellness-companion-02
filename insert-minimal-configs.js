...
async function insertMinimalConfigurations() {
  console.log('🚀 Inserindo configurações mínimas de IA...\n');
  ...
  try {
    ...
    if (checkError) {
      ...
      checkData.forEach(config => {
        console.log(`   - ${config.functionality}: ${config.is_enabled ? '✅ Ativo' : '❌ Inativo'}`);
      });
    }
